...
mod tests {
    ...
    fn test_human_vs_human_fight() {
        ...
        let mut manny = HumanImpl::new();
        let mut connie = HumanImpl::new();

        match human_vs_human_fight(ref manny, ref connie) {
            Result::Ok(()) => {
                assert!(manny.get_health() == 90);
                assert!(connie.get_health() == 90);
                assert!(manny.is_alive());
                assert!(connie.is_alive());
            },
            Result::Err(_e) => assert!(false),
        }
    }

    fn fight<T1, T2, +Action<T1>, +Action<T2>, +Drop<T1>, +Drop<T2>>(ref fighter_1: T1, ref fighter_2: T2) -> Result<(), ByteArray> {
        if !fighter_1.is_alive() || !fighter_2.is_alive() {
            return Result::Err("One of the fighters is already dead.");
        }
    ...
    fn test_fight() {
        ...
        match fight(ref manny, ref gorbag) {
            Result::Ok(()) => {
                assert!(manny.get_health() == 90);
                assert!(gorbag.get_health() == 90);
                assert!(manny.is_alive());
