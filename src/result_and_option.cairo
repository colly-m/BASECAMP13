// Module that builds on top of casting where the enum option introduced
// Compares the options and results the enums to be used individually
...
mod tests {
	...
	match result {
	    Result::Err(error) => assert!(error == "Cant't divide by zero"),
	    Result::Ok(_) => assert!(false),
	}
    }
